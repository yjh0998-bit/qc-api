...
PROGRESS = {0:'접수대기',1:'의뢰대기',2:'의뢰서작성중',3:'결과입력대기',4:'결과입력중',5:'결재대기',6:'검사완료'}
TEST_TYPE = {0:'사전검사',1:'입고검사',2:'생산검사',3:'관리검사',4:'협조시험',5:'기타'}
PASS_FAIL = {0:'부적합',1:'적합',2:'미정'}
...
class Handler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        try:
            ...
            if path == '/':
            ...
            elif path == '/col_receipt':
                cur.execute('DESCRIBE receipt' if False else 'DESCRIBE 접수')
                ...
                rows = cur.fetchall()
                for r in rows:
                    r['진행상황_text'] = PROGRESS.get(r.get('진행상황'), '알수없음')
                    r['시험구분_text'] = TEST_TYPE.get(r.get('시험구분'), '알수없음')
                    r['적합여부_text'] = PASS_FAIL.get(r.get('적합여부'), '미정')
